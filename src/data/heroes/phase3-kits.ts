...
function vfx(archetype: VfxArchetype, color: string, color2?: string, scale = 0.9): AbilityDef['vfx'] {
  return { archetype, color, color2, scale };
}

/** Hand-tune signature marquee VFX (color / color2 / scale / sound) without changing archetype. */
function sig(a: AbilityDef, color: string, color2: string, scale: number, sound?: SoundArchetype): AbilityDef {
  return { ...a, vfx: { ...a.vfx!, color, color2, scale }, ...(sound !== undefined ? { sound } : {}) };
}
...
export const AUTHORED_PHASE3_KITS: Partial<Record<HeroDef['id'], Kit>> = {
  ...
  'shadow-fiend': kit(
    ...
    sig(withAnim(ultArea('sf-requiem', 'Requiem of Souls', '#d84a32', [{ kind: 'damage', dtype: 'magical', amount: 'damage', target: 'enemies-in-radius', radius: 'radius' }, { kind: 'status', status: 'fear', duration: 'disable', target: 'enemies-in-radius', radius: 'radius' }], 'storm'), 'global-cast'), '#d84a32', '#5f0b0b', 1.5, 'void')
  ),
  riki: kit(
    ...
    sig(withAnim(ultArea('riki-tricks', 'Tricks of the Trade', '#6a4cff', [{ kind: 'status', status: 'cyclone', duration: 1.5, target: 'self' }, { kind: 'damage', dtype: 'physical', amount: 'damage', target: 'enemies-in-radius', radius: 'radius' }], 'channel'), 'channel-loop'), '#6a4cff', '#c8b8ff', 0.95)
  ),
  'bounty-hunter': kit(
    ...
    sig(ultTarget('bh-track', 'Track', '#d99a28', [{ kind: 'status', status: 'break', duration: 'duration', target: 'target' }, { kind: 'statmod', mods: { moveSpeedPct: 20 }, duration: 'duration', target: 'self' }]), '#d99a28', '#f2df7a', 0.95)
  ),
  lion: kit(
    ...
    sig(ultTarget('lion-finger', 'Finger of Death', '#a347ff', []), '#a347ff', '#ffca66', 1.35, 'fire')
  ),
  'winter-wyvern': kit(
    ...
    sig(ultTarget('ww-winters-curse', "Winter's Curse", '#9fe8ff', [{ kind: 'status', status: 'taunt', duration: 'disable', target: 'target' }]), '#9fe8ff', '#3d75b8', 1.2, 'frost')
  ),
  'sand-king': kit(
    ...
    sig(withAnim(ultArea('sk-epicenter', 'Epicenter', '#d9a441', [{ kind: 'repeat', count: 6, interval: 0.45, effects: [{ kind: 'damage', dtype: 'magical', amount: 'damage', target: 'enemies-in-radius', radius: 'radius' }, { kind: 'status', status: 'slow', duration: 1, target: 'enemies-in-radius', radius: 'radius', params: { moveSlowPct: 35 } }] }], 'storm'), 'channel-loop'), '#d9a441', '#fff0a8', 1.55, 'storm')
  ),
  'nyx-assassin': kit(
    ...
    sig(ultTarget('nyx-vendetta', 'Vendetta', '#5d3b9a', [{ kind: 'status', status: 'break', duration: 'duration', target: 'target' }], 'physical'), '#5d3b9a', '#c4a3ff', 0.85, 'void')
  ),
  medusa: kit(
    ...
    sig(ultArea('medusa-stone-gaze', 'Stone Gaze', '#2aa86b', [{ kind: 'exotic', id: 'stone-gaze', params: { heroId: 'medusa' } }, { kind: 'status', status: 'frozen', duration: 'disable', target: 'enemies-in-radius', radius: 'radius' }], 'global-mark'), '#2aa86b', '#d8f5a2', 1.45)
  ),
  viper: kit(
    ...
    sig(ultTarget('viper-viper-strike', 'Viper Strike', '#6fc247', [{ kind: 'status', status: 'slow', duration: 'duration', target: 'target', params: { moveSlowPct: 55, dotDps: 'damage', dotType: 'magical' } }]), '#6fc247', '#c8ff7a', 1.15)
  ),
  kunkka: kit(
    ...
    sig(ultArea('kunkka-ghostship', 'Ghostship', '#2a6d9a', [{ kind: 'damage', dtype: 'magical', amount: 'damage', target: 'enemies-in-radius', radius: 'radius' }, { kind: 'status', status: 'stun', duration: 'disable', target: 'enemies-in-radius', radius: 'radius' }, { kind: 'statmod', mods: { damageTakenReductionPct: 35 }, duration: 'duration', target: 'allies-in-radius', radius: 'radius' }]), '#2a6d9a', '#e8d8a0', 1.45, 'storm')
  ),
  tidehunter: kit(
  ...
  slardar: kit(
    ...
    sig(nova('slardar-crush', 'Slithereen Crush', '#cbb8ff', [{ kind: 'status', status: 'stun', duration: 1.2, target: 'enemies-in-radius', radius: 'radius' }], 'physical'), '#8050d8', '#cbb8ff', 1.35, 'roar'),
  ...
  'naga-siren': kit(
    ...
    sig(withAnim(ultArea('naga-song', 'Song of the Siren', '#4bb8d8', [{ kind: 'status', status: 'sleep', duration: 'disable', target: 'enemies-in-radius', radius: 'radius' }], 'channel'), 'channel-loop'), '#4bb8d8', '#f0d08a', 1.4, 'frost')
  ),
  slark: kit(
    ...
    sig(ultArea('slark-shadow-dance', 'Shadow Dance', '#2a6f8f', [{ kind: 'status', status: 'invis', duration: 'duration', target: 'self' }, { kind: 'statmod', mods: { hpRegen: 40, moveSpeedPct: 30 }, duration: 'duration', target: 'self' }]), '#2a6f8f', '#9bdcff', 0.9, 'void')
  ),
  lifestealer: kit(
    ...
    sig(ultTarget('ls-infest', 'Infest', '#b24a32', [{ kind: 'heal', amount: 'damage', target: 'self' }, { kind: 'status', status: 'slow', duration: 'disable', target: 'target', params: { moveSlowPct: 45 } }], 'physical'), '#b24a32', '#e8b082', 1.15, 'roar')
  ),
  undying: kit(
    ...
    sig(ultArea('undying-flesh-golem', 'Flesh Golem', '#5aa36a', [{ kind: 'statmod', mods: { maxHp: 450, damage: 55 }, duration: 'duration', target: 'self' }, { kind: 'status', status: 'slow', duration: 'disable', target: 'enemies-in-radius', radius: 'radius', params: { moveSlowPct: 35 } }]), '#5aa36a', '#d8e8aa', 1.3, 'roar')
  ),
  doom: kit(
    ...
    sig(ultTarget('doom-doom', 'Doom', '#c23a1f', [{ kind: 'status', status: 'silence', duration: 'duration', target: 'target' }, { kind: 'status', status: 'disarm', duration: 'duration', target: 'target' }]), '#c23a1f', '#ff9a3a', 1.2, 'void')
  ),
  'wraith-king': kit(
    ...
    sig(ultArea('wk-reincarnation', 'Reincarnation', '#41d878', [{ kind: 'exotic', id: 'reincarnation', params: { heroId: 'wraith-king' } }, { kind: 'heal', amount: 'damage', target: 'self' }, { kind: 'status', status: 'slow', duration: 'disable', target: 'enemies-in-radius', radius: 'radius', params: { moveSlowPct: 55 } }]), '#41d878', '#d8ffd8', 1.45, 'roar')
  ),
  'night-stalker': kit(
    ...
    sig(ultArea('ns-dark-ascension', 'Dark Ascension', '#1b2a58', [{ kind: 'statmod', mods: { damage: 65, moveSpeedPct: 25, visionPct: 45 }, duration: 'duration', target: 'self' }, { kind: 'status', status: 'fear', duration: 'disable', target: 'enemies-in-radius', radius: 'radius' }]), '#1b2a58', '#7a8cff', 1.25, 'roar')
  ),
  invoker: kit(
    buff('invoker-orbs', 'Quas Wex Exort', '#f8d36a', { spellAmpPct: 10, manaRegen: 2, attackSpeed: 25 }, { duration: [8, 9, 10, 11] }),
    sig(withAnim(ground('invoker-sun-strike', 'Sun Strike', '#f8d36a', [], 'pure'), 'global-cast'), '#f8d36a', '#a8e8ff', 1.45, 'storm'),
  ...
  silencer: kit(
    ...
    sig(ultArea('silencer-global-silence', 'Global Silence', '#b78cff', [{ kind: 'status', status: 'silence', duration: 'disable', target: 'enemies-in-radius', radius: 'radius' }], 'global-mark'), '#b78cff', '#e8e8ff', 1.5, 'void')
  ),
  'outworld-destroyer': kit(
    ...
    sig(target('od-astral-imprisonment', 'Astral Imprisonment', '#d8f7ff', [status('cyclone', 1.8), { kind: 'mana', op: 'restore', amount: 'damage', target: 'self' }]), '#64d8ff', '#d8f7ff', 1.15, 'void'),
  ...
  'skywrath-mage': kit(
    ...
    sig(withAnim(ultArea('sky-mystic-flare', 'Mystic Flare', '#7ec8ff', [{ kind: 'repeat', count: 4, interval: 0.35, effects: [{ kind: 'damage', dtype: 'magical', amount: 'damage', target: 'enemies-in-radius', radius: 'radius' }] }], 'channel'), 'channel-loop'), '#7ec8ff', '#f7e39a', 1.35, 'storm')
  ),
  tinker: kit(
    ...
    sig(ultArea('tinker-rearm', 'Rearm', '#e05040', [{ kind: 'exotic', id: 'rearm', params: { heroId: 'tinker' } }, { kind: 'exotic', id: 'refresh-cooldowns', params: { heroId: 'tinker' } }, { kind: 'statmod', mods: { spellAmpPct: 18 }, duration: 'duration', target: 'self' }], 'global-mark'), '#e05040', '#f8e07a', 1.1)
  ),
  enchantress: kit(
    ...
    sig(buff('ench-attendants', "Nature's Attendants", '#345d2f', { hpRegen: 28 }, { duration: [6, 7, 8, 9] }), '#73d86b', '#fff2a6', 1.1, 'heal'),
  ...
  chen: kit(
    ...
    sig(ultArea('chen-hand-of-god', 'Hand of God', '#f4e4a0', [{ kind: 'heal', amount: 'damage', target: 'allies-in-radius', radius: 'radius' }], 'global-mark'), '#f4e4a0', '#ffffff', 1.35, 'heal')
  ),
  'natures-prophet': kit(
    ...
    sig(ultArea('np-wrath', 'Wrath of Nature', '#4dbd62', [{ kind: 'repeat', count: 5, interval: 0.25, effects: [{ kind: 'damage', dtype: 'magical', amount: 'damage', target: 'random-enemy-in-radius', radius: 'radius' }] }], 'chain'), '#4dbd62', '#d8ffd8', 1.4)
  ),
  beastmaster: kit(
  ...
  broodmother: kit(
    ...
    sig(summonSpell('brood-spiderlings', 'Spawn Spiderlings', '#d38cff', spider, 'count'), '#5b2b72', '#d38cff', 1.15, 'summon'),
  ...
  warlock: kit(
    ...
    sig(ultArea('warlock-chaotic-offering', 'Chaotic Offering', '#9b2d2d', [{ kind: 'damage', dtype: 'magical', amount: 'damage', target: 'enemies-in-radius', radius: 'radius' }, { kind: 'status', status: 'stun', duration: 'disable', target: 'enemies-in-radius', radius: 'radius' }, { kind: 'summon', summon: golem, count: 1, at: 'self' }], 'summon-pop'), '#9b2d2d', '#f2c06b', 1.45, 'summon')
  ),
  visage: kit(
    ...
    sig(ultArea('visage-familiars', 'Summon Familiars', '#77778f', [{ kind: 'summon', summon: familiar, count: 2, at: 'self' }, { kind: 'status', status: 'stun', duration: 'disable', target: 'enemies-in-radius', radius: 'radius' }], 'summon-pop'), '#77778f', '#c8c8e8', 1.25, 'summon')
  ),
  magnus: kit(
  ...
  'elder-titan': kit(
    ...
    sig(withAnim(ultArea('et-earth-splitter', 'Earth Splitter', '#8a6a4a', [{ kind: 'damage', dtype: 'pure', amount: 'damage', target: 'enemies-in-radius', radius: 'radius' }, { kind: 'status', status: 'slow', duration: 'disable', target: 'enemies-in-radius', radius: 'radius', params: { moveSlowPct: 55 } }], 'wall'), 'ground-slam'), '#8a6a4a', '#e0d0b0', 1.45)
  ),
  tiny: kit(
    ground('tiny-avalanche', 'Avalanche', '#9a9a8a', [{ kind: 'status', status: 'stun', duration: 0.8, target: 'target' }], 'physical'),
    sig(target('tiny-toss', 'Toss', '#e0e0d0', [{ kind: 'displace', mode: 'knockback', target: 'target', toward: 'away-from-caster', distance: 360, speed: 1000 }], 'physical'), '#9a9a8a', '#e0e0d0', 1.15),
  ...
  'treant-protector': kit(
    ...
    sig(ultArea('treant-overgrowth', 'Overgrowth', '#3f7a3a', [{ kind: 'damage', dtype: 'magical', amount: 'damage', target: 'enemies-in-radius', radius: 'radius' }, { kind: 'status', status: 'root', duration: 'disable', target: 'enemies-in-radius', radius: 'radius' }], 'wall'), '#3f7a3a', '#d8f0a8', 1.4)
  ),
  'centaur-warrunner': kit(
    ...
    sig(ultArea('centaur-stampede', 'Stampede', '#9a5a32', [{ kind: 'statmod', mods: { moveSpeedPct: 45 }, duration: 'duration', target: 'allies-in-radius', radius: 'radius' }, { kind: 'status', status: 'stun', duration: 0.8, target: 'enemies-in-radius', radius: 'radius' }]), '#9a5a32', '#f0c090', 1.35, 'roar')
  ),
  'storm-spirit': kit(
    ...
    sig(withAnim(ultArea('storm-ball-lightning', 'Ball Lightning', '#58a8ff', [{ kind: 'displace', mode: 'blink', target: 'self', toward: 'point', distance: 800 }, { kind: 'damage', dtype: 'magical', amount: 'damage', target: 'enemies-in-radius', radius: 'radius' }], 'global-mark'), 'dash'), '#58a8ff', '#ffffff', 1.25, 'storm')
  ),
  'ember-spirit': kit(
    ...
    sig(withAnim(ultArea('ember-fire-remnant', 'Fire Remnant', '#ff6b2a', [{ kind: 'displace', mode: 'blink', target: 'self', toward: 'point', distance: 850 }, { kind: 'damage', dtype: 'magical', amount: 'damage', target: 'enemies-in-radius', radius: 'radius' }]), 'dash'), '#ff6b2a', '#ffd27a', 1.15, 'fire')
  ),
  spectre: kit(
    ...
    sig(withAnim(ultArea('spectre-haunt', 'Haunt', '#7a67ff', [{ kind: 'damage', dtype: 'magical', amount: 'damage', target: 'enemies-in-radius', radius: 'radius' }, { kind: 'status', status: 'fear', duration: 'disable', target: 'enemies-in-radius', radius: 'radius' }], 'global-mark'), 'summon-gesture'), '#7a67ff', '#d8d0ff', 1.4, 'void')
  ),
  'faceless-void': kit(
  ...
  phoenix: kit(
    ...
    sig(withAnim(ultArea('phoenix-supernova', 'Supernova', '#ff7a30', [{ kind: 'summon', summon: egg, count: 1, at: 'self' }, { kind: 'damage', dtype: 'magical', amount: 'damage', target: 'enemies-in-radius', radius: 'radius' }, { kind: 'status', status: 'stun', duration: 'disable', target: 'enemies-in-radius', radius: 'radius' }], 'global-mark'), 'summon-gesture'), '#ff7a30', '#ffe07a', 1.5, 'fire')
  ),
  io: kit(
